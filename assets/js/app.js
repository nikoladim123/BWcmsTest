...
var paralxSpeed = document.getElementsByClassName('paralxSpeed');

var paralaxSpeedValue = 5;
function flowersParalaxFun() {
  flowersParalax.style.top =  window.scrollY/paralaxSpeedValue + "px";
}
paralxSpeed[0].addEventListener('input',(e)=>{
  var el = e.target.value;
  if(el >= 0 && el <= 10){
    paralaxSpeedValue = el;
    console.log(el);
  }
});
...
// edit page buton
var editLayer = document.getElementsByClassName('editLayer');
var editLayerOnOff = document.getElementsByClassName('editLayerOnOff');

var editButonbMod = 1
editLayerOnOff[0].addEventListener('click',()=>{
  if(editButonbMod % 2 === 0 ){
    editLayer[0].style.display = 'none';
  }
  if(editButonbMod % 2 === 1 ){
...
var colorPickerInput = document.getElementsByClassName('colorPickerInput');
var colorPickerClassChange = document.getElementsByClassName('colorPickerClassChange');


colorPickerInput[0].addEventListener('change',(e)=>{
  colorPickerClassChange[0].style.backgroundColor = e.target.value;
  colorPickerClassChange[1].style.backgroundColor = e.target.value;
  colorPickerClassChange[2].style.backgroundColor = e.target.value;
  console.log(e.target);
})

//opacity picker
var opacityPicker = document.getElementsByClassName('opacityPicker');

opacityPicker[0].addEventListener('input',(e)=>{
  colorPickerClassChange[0].style.opacity = e.target.value;
  colorPickerClassChange[1].style.opacity = e.target.value;
  colorPickerClassChange[2].style.opacity = e.target.value;
})

// change name
var logoNameEditInput = document.getElementsByClassName('logoNameEditInput');
var initialEdit = document.getElementsByClassName('initialEdit');
var hOneEdit =  document.getElementsByClassName('hOneEdit');
var initContainer =  document.getElementsByClassName('initContainer');

var buttonEditLeft = document.getElementsByClassName('buttonEditLeft');
var buttonEditTop = document.getElementsByClassName('buttonEditTop');
var buttonEditRight = document.getElementsByClassName('buttonEditRight');
var buttonEditBottom = document.getElementsByClassName('buttonEditBottom');


logoNameEditInput[0].addEventListener('input',(e)=>{
  if(typeof e.target.value[0] !== 'undefined'){
    initialEdit[0].innerText = e.target.value[0];
    hOneEdit[0].innerHTML = `<span>${e.target.value[0]}</span>${e.target.value.substring(1, 15)} `
  }
});

// left/right
var buttonLeftValue = -1.1176;
buttonEditLeft[0].addEventListener('click',()=>{
  buttonLeftValue -= 0.1;
  initContainer[0].style.left = buttonLeftValue + 'vw';
});

buttonEditRight[0].addEventListener('click',()=>{
  buttonLeftValue += 0.1;
  initContainer[0].style.left = buttonLeftValue + 'vw';
});
// top/bot
var buttonTopValue = 2.71;
buttonEditTop[0].addEventListener('click',()=>{
  buttonTopValue -= 0.1;
  initContainer[0].style.top = buttonTopValue + 'vw';
  console.log(11);
});

buttonEditBottom[0].addEventListener('click',()=>{
  buttonTopValue += 0.1;
  initContainer[0].style.top = buttonTopValue + 'vw';
});
...
// hoverOverlay
var hoverOverlay = document.getElementsByClassName('hoverOverlay');
var yellowOverlay = document.getElementsByClassName('yellowOverlay');

hoverOverlay[0].addEventListener('input',(e)=>{
  yellowOverlay[0].style.backgroundColor = e.target.value;
  yellowOverlay[1].style.backgroundColor = e.target.value;
  yellowOverlay[2].style.backgroundColor = e.target.value;
});

//topRubyBnd
var topRubyBnd = document.getElementsByClassName('topRubyBnd');
var bndColorEdit = document.getElementsByClassName('bndColorEdit');

bndColorEdit[0].addEventListener('input',(e)=>{
  topRubyBnd[0].style.backgroundColor = e.target.value;
});

// body
var body = document.getElementsByTagName('body')[0];
var bodyColorInput = document.getElementsByClassName('bodyColorInput');

bodyColorInput[0].addEventListener('input',(e)=>{
